...
export const astroAssetsOptimizer: ImagesOptimizer = async (image, breakpoints, _width, _height, format = undefined) => {
  ...
  // ✅ CRITICAL: Never call Astro getImage() for "/public" style paths like "/blog_images/..."
  // That pipeline can throw MissingImageDimension.
  ...
  return Promise.all(
    breakpoints.map(async (w: number) => {
      ...
      return {
        src: result?.src,
        width: (result?.attributes?.width as number) ?? w,
        height: result?.attributes?.height as number | undefined,
      };
    })
  );
};
...
export const unpicOptimizer: ImagesOptimizer = async (image, breakpoints, width, height, format = undefined) => {
  ...
  return Promise.all(
    breakpoints.map(async (w: number) => {
      ...
      return { src: String(url), width: w, height: _height };
    })
  );
};
...
export async function getImagesOptimized(
  ...
  // ✅ Imported assets: infer from metadata
  ...
  // ✅ Backup safety: if a public path sneaks in, ensure dimensions exist
  if (typeof image === 'string' && image.startsWith('/') && layout !== 'fullWidth') {
    if (!width && !height) {
      width = 900;
